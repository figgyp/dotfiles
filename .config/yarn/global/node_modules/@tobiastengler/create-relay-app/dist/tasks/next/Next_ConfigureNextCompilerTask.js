...
export class Next_ConfigureNextCompilerTask extends TaskBase {
    constructor(context) {
        ...
        this.message = `Configure Next.js compiler`;
    }
    isEnabled() {
    ...
    async run() {
        ...
        traverse.default(ast, {
            AssignmentExpression: (path) => {
                ...
                let compiler_Prop = objExp.properties.find((p) => t.isObjectProperty(p) && t.isIdentifier(p.key) && p.key.name === "compiler");
                if (!compiler_Prop) {
                ...
                if (this.context.artifactPath) {
                    relay_ObjProps.push(t.objectProperty(t.identifier("artifactDirectory"), t.stringLiteral(this.context.artifactPath.rel)));
                }
                const compiler_relayProp = compiler_Prop.value.properties.find((p) => t.isObjectProperty(p) && t.isIdentifier(p.key) && p.key.name === "relay");
                if (compiler_relayProp && t.isObjectExpression(compiler_relayProp.value)) {
                    ...
                    // with the new ones.
                    compiler_relayProp.value = t.objectExpression(mergeProperties(compiler_relayProp.value.properties, relay_ObjProps));
                }
                else {
                    // We do not yet have a "relay" propery, so we add it.
                    compiler_Prop.value.properties.push(t.objectProperty(t.identifier("relay"), t.objectExpression(relay_ObjProps)));
                }
                path.skip();
            },
        });
